 ...
 * and synchronising inventory across variants.
 *
...
export default function SyncVariantModal({ variantId, onClose, onUpdate }) {
  ...
  /**
   * Updates child inventory based on master inventory and child ratio
   * IMPORTANT: Updates qtyold metafield FIRST, then updates inventory
   *
   * @param {string} childId - ID of the child variant
   * @param {number} masterQty - Current inventory of the master variant
   * @param {number} childRatio - Ratio value for the child (qtymanagement)
   * @returns {Promise<number>} The new child inventory quantity
   */
  const updateChildInventory = async (childId, masterQty, childRatio) => {
    try {
      console.log(`Updating inventory for child ${childId} based on master qty ${masterQty} and ratio ${childRatio}`);

      // Calculate new inventory for child: masterQty / childRatio
      const newChildQty = Math.floor(masterQty / (childRatio || 1));
      console.log(`New inventory calculated for child: ${newChildQty}`);

      // 1. FIRST: Update qtyold metafield - CRITICAL to do this BEFORE inventory change
      console.log(`Updating qtyold to ${newChildQty} BEFORE changing inventory`);
      const metafieldUpdateResponse = await fetch('/api/update-variant-metafield', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          key: 'qtyold',
          value: String(newChildQty),
          type: "number_integer"
        })
      });

      if (!metafieldUpdateResponse.ok) {
        console.error("Error updating the child qtyold metafield:", await metafieldUpdateResponse.text());
        throw new Error("Error updating the qtyold metafield which must precede the inventory change");
      }

      // 2. SECOND: Update actual inventory
      console.log(`qtyold updated successfully, now updating inventory to ${newChildQty}`);
      const inventoryUpdateResponse = await fetch("/api/update-inventory", {
        ...
        body: JSON.stringify({
          variantId: childId,
          newQuantity: newChildQty
        })
      });

      if (!inventoryUpdateResponse.ok) {
        console.error("Error updating child inventory:", await inventoryUpdateResponse.text());
        throw new Error("Error updating child inventory");
      }

      console.log(`Metafield qtyold and inventory updated for child ${childId} to ${newChildQty}`);
      return newChildQty;
    } catch (error) {
      console.error("Error in updateChildInventory:", error);
      throw error;
    }
  };
              ...
              const childPromises = updatedChildren.map(async (childId) => {
                try {
                  ...
                  childDetails[childId] = {
                    ...
                    image:
                      childProduct.image?.originalSrc ||
                      childProduct.product?.images?.edges?.[0]?.node?.originalSrc ||
                      null,
                  ...
                  if (childProduct.ratioMetafield?.value) {
                    ...
                    const qtyMetaEdge = childProduct.metafields.edges.find(
                      edge =>
                        edge.node?.namespace === "projektstocksyncqtymanagement" &&
                  ...
                  if (childProduct.parentMasterMetafield?.value) {
                    ...
                    const parentMasterEdge = childProduct.metafields.edges.find(
                      edge =>
                        edge.node?.namespace === "projektstocksyncparentmaster" &&
      ...
      try {
        ...
        if (
          ...
          const parentMasterEdge = v.metafields.edges.find(
            edge =>
              edge.node?.namespace === "projektstocksyncparentmaster" && edge.node?.key === "parentmaster"
          );
          if (
   ...
   * Ensures displayed data is synchronised with the backend
   *
  ...
  /**
   * Simulates successful synchronisation with UI animation, without actually
   * performing inventory modifications.
   *
   * @returns {Promise<boolean>} - True if the animation completed successfully
   */
  const simulateSyncProcess = async () => {
    try {
      console.log("START: SIMULATING SYNCHRONISATION PROCESS");

      // Start animation - will run for exactly 8 seconds
      ...
      let animationTimer = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          setTimeout(() => {
            setIsProcessing(false);
            setShowSuccessAnimation(false);
            setSuccess("Synchronisation completed");
      ...
      console.log("END: SYNCHRONISATION PROCESS SIMULATION COMPLETE");
      return true;
    } catch (error) {
      console.error("ERROR IN SYNCHRONISATION SIMULATION:", error);
      setShowSuccessAnimation(false);
      setIsProcessing(false);
      return false;
    }
   ...
   * Updates the children metafield, sets the parent-master relationship
   * and synchronises the child's inventory based on master inventory and ratio
   *
  ...
  const handleAddChild = async (childId) => {
    try {
      ...
      if (!res.ok) {
        const errText = await res.text();
        throw new Error(errText || 'Error adding child variant');
      }

      // Update local state
      setChildren(updatedChildren);

      // Set default ratio of 1 for new child
      const defaultRatio = 1;

      // Set qtymanagement metafield with default ratio
      await fetch('/api/update-variant-metafield', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          variantId: childId,
          ...
          key: 'qtymanagement',
          value: String(defaultRatio),
          type: 'number_integer'
        })
      });

      // Get current master inventory
      const currentMasterQty = Number(inventory);

      // Update child inventory based on master inventory and ratio
      await updateChildInventory(childId, currentMasterQty, defaultRatio);

      // Close the add child modal first
      ...
      try {
          ...
          if (v.childrenMetafield?.value) {
            try {
              ...
              await Promise.all(updatedChildren.map(async (cId) => {
                await refreshChildData(cId);
              }));
            } catch (e) {
              console.error("Error parsing updated child data:", e);
            }
          }
        ...
        console.error("Error updating modal data:", refreshError);
      }

      setSuccess("Child variant successfully added and synchronised.");

      console.log(`==== END: ADD CHILD VARIANT COMPLETED ====`);
      return true;
    } catch (err) {
      ...
      setShowSuccessAnimation(false);
      setProcessingProgress(0);
      console.error("Error adding child variant:", err);
      setError(err.message);
      return false;
    }
  };
  ...
  /**
   * Updates the master ratio for a child variant and recalculates its inventory
   *
   * @param {string} childId - The ID of the child variant
  ...
  async function handleUpdateMasterRatio(childId, newRatio) {
    try {
      ...
      // Update child inventory based on master inventory and new ratio
      await updateChildInventory(childId, currentMasterQty, ratioNumber);
        ...
        if (refreshRes.ok) {
          ...
          if (v.childrenMetafield?.value) {
            try {
              ...
              // Re-fetch child details for all children
              await Promise.all(
                updatedChildren.map(async (cId) => {
                  await refreshChildData(cId);
                })
              );
            } catch (e) {
   ...
   * Shows a loading animation to simulate background processing
   */
      ...
      if (!isChild) {
        ...
        const masterRes = await fetch("/api/update-variant-metafield", {
          ...
          body: JSON.stringify({
            ...
            key: "master",
            value: master ? "true" : "false",  // Mantén esto como string
            type: "boolean"  // Cambia el tipo a texto en lugar de boolean
          }),
        });
              ...
              {isProcessing ? (
                <>
                  ...
                  >
                    ...
                    >
                      {/* Ensure the percentage is always an integer */}
                  ...
                  >
                    Changes are being processed in the background. Please wait...
                  </div>

                  <div style={{ width: "100%", maxWidth: "400px" }}>
                    <ProgressBar progress={Math.floor(processingProgress)} size="large" />
                  </div>
                </>
              ) : (
                <>
                                  ...
                                  >
                                    <div
                                      style={{
                                        ...
                                        borderRadius: "12px",
                                        backgroundColor:
                                          childInfo.inventory && childInfo.inventory > 0 ? "#e3f1df" : "#fbeae5",
                                  ...
                                  >
                                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                                      <TextField
                                        ...
                                        type="number"
                                        value={
                                          masterRatios[childId] !== undefined
                                            ? String(masterRatios[childId])
                                            : ""
                                        }
